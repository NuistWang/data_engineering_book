...
```mermaid
flowchart TD
    A[用户/外部触发] --> B[Planner 规划层]
    B --> C[Tool Executor 执行层]
    C --> D[Verifier 验证层]
    D --> E{Human Gate<br>人工闸门}
    E -->|通过| F[数据变更生效]
    E -->|驳回| B
    B <--> G[Memory 记忆层]
    C --> H[Lineage 血缘层]
...
flowchart TD
    A[用户提交任务] --> B{风险评估}
    B -->|低风险 L1| C[Agent 分析并输出建议]
    C --> D[人工审阅建议]
    D --> E[人工执行]
    B -->|中风险 L2| F[Agent 生成操作草案]
    F --> G[Verifier 沙箱验证]
    G --> H[人工确认执行]
    H --> I[Agent 执行并记录]
    B -->|高风险 L3| J[Agent 生成方案]
    J --> K[多人审批]
    K --> L[Agent 执行]
    L --> M[Verifier 验证]
    M --> N[人工验收]
    B -->|自治 L4| O[Agent 自主执行]
    O --> P[Lineage 记录]
    P --> Q[定期人工审计]
```
